 ...
 *
 * ### :material-notes: calibration philosophy
...
/// Values above 1 darken the vertical edges faster.
...
/// Minimum HSV saturation admitted to the bloom mip chain.
...
/// Warm/cool RGB amplitude for PostFXParams::ComputeGradingParams.
constexpr float GRADING_TINT_AMPLITUDE = 0.06f;

/// Radial R/B sampling offset in UV units; zero at screen center.
constexpr float CA_STRENGTH = 0.007f;

/// Input threshold above which highlights approach 1 asymptotically.
...
/// Multiplied by fadeAlpha during the panel animation.
constexpr float DIALOGUE_PANEL_FILL_ALPHA = 0.85f;

/**
 * @brief Panel border color (#3a4555).
 *
 * 1px lighter slate around the rect for definition; full opacity (modulated only by fadeAlpha
 * during scale-in).
 */
...
/**
 * @brief Speaker ribbon text fill color.
 *
 * same rationale as body text: light fill + black outline reads on any accent background without a
 * luminance flip.
 */
...
/**
 * @brief Fallback accent color when SampleDominantNonSkinColor finds nothing usable.
 *
...
/**
 * @brief Default transition duration in real seconds.
 *
 * Used by manual weather commands, forecast fronts and the manual sky overlay.
 * Night events use WEATHER_EVENT_TRANSITION_SECONDS.
 */
constexpr float WEATHER_TRANSITION_SECONDS = 3.0f;

/**
 * @brief Frequency-space cutoff for blended lightning (hz).
 *
 * blended frequencies below this map to interval 0 ("off") instead of a near-infinite interval.
 */
constexpr float WEATHER_LIGHTNING_FREQ_EPS = 0.005f;

/**
 * @brief Fog multiplier decay in real seconds.
 *
 * After fog ends, retain the multiplier while existing puffs expire.
 * The decay window matches the maximum puff lifetime.
 */
constexpr float WEATHER_FOG_HOLD_DECAY_SECONDS = 18.0f;

/**
 * @brief Half-width (game hours) of the dawn/dusk ramp applied to the sky-color
...
/**
 * @brief Direction wander sine period, real seconds (slow, so drift reads as
...
/**
 * @brief Real-seconds duration for night-event transitions (night itself is only
 * ~9 real seconds at the 24 s day, so events ramp faster than fronts).
 */
constexpr float WEATHER_EVENT_TRANSITION_SECONDS = 3.0f;

/**
 * @brief Nominal weather-front length in in-game days (~96 real seconds at the 24 s day).
 *
 * boundaries jitter by +/-1 day per front.
 */
constexpr int WEATHER_FRONT_LENGTH_DAYS = 4;

/**
 * @brief Probability that any given night hosts a special event (aurora / meteor

...
// Feather alpha to zero at every border; Gaussian falloff alone leaves an edge.
...
std::vector<unsigned char> BuildCurtainPixels(int width, int height)
{
    ...
    {
        const float ny = static_cast<float>(y) / heightDenom;
        // Combine a narrow bright core with a broad translucent halo.
        const float dv = (ny - 0.5f) * 2.0f;
        ...
        for (int x = 0; x < width; ++x)
        {
            ...
            // Small ray variation avoids visible segment boundaries.
...
std::vector<unsigned char> BuildBeamPixels(int width, int height)
{
    ...
    {
        const float ny = static_cast<float>(y) / heightDenom;
        // Center the beam glow and fade both ends.
        const float dv = (ny - 0.5f) * 2.0f;
        ...
        for (int x = 0; x < width; ++x)
        {
            const float nx = static_cast<float>(x) / widthDenom;
            const float edge = nx * 2.0f - 1.0f;
            const float horizontal = std::exp(-edge * edge * 2.4f);  // Feather the sides to zero.
